...
public interface RESTClientEmissionService {
		...
		@PostMapping("/api/updateEmission/{idEmis}/{status}/{refCamcis}/{accountNumber}")
		ResponseEntity<RetPaiFiscalis[]> updateEmission(@PathVariable ("idEmis") Long idEmis, @PathVariable ("status") Statut status, @PathVariable ("refCamcis") String refCamcis, @PathVariable ("accountNumber") String accountNumber, @RequestBody PaymentDTO payment);
		...
		@PostMapping("/api/emission/reconciled")
		List<String> notifyReconciledEmission(@RequestBody List<String> references);
}
