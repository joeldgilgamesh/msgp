...
class EmissionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Emission entity")
        ...
        .repeat(2) {
            ...
            .get("/services/spminfimsgp/api/emissions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new emission")
            .post("/services/spminfimsgp/api/emissions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "refEmi":"SAMPLE_TEXT"
                , "amount":null
                , "codeContribuable":"SAMPLE_TEXT"
                , "status":"DRAFT"
                , "idOrganisation":null
                , "idTypeEmisId":null
                }""")).asJson
            ...
            .repeat(5) {
                ...
                .get("/services/spminfimsgp${new_emission_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created emission")
            .delete("/services/spminfimsgp${new_emission_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
