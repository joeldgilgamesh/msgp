...
class PaymentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Payment entity")
        ...
        .repeat(2) {
            ...
            .get("/services/spminfimsgp/api/payments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new payment")
            .post("/services/spminfimsgp/api/payments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "code":"SAMPLE_TEXT"
                , "meansOfPayment":"MOBILE_MONEY"
                , "statut":"DRAFT"
                , "amount":null
                , "idEmission":null
                , "idRecette":null
            ...
            .repeat(5) {
                ...
                .get("/services/spminfimsgp${new_payment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created payment")
            .delete("/services/spminfimsgp${new_payment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
